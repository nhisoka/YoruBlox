...
public:
    ...
    void initialize(const wchar_t* driver_name, int target_process_id);
    ...
    {
        T buffer{};
        read_memory(address, (uintptr_t)&buffer, sizeof(T));
        return buffer;
    }
    template<typename T> void write(uintptr_t address, T value)
    {
        write_memory(address, (uintptr_t)&value, sizeof(T));
    }
